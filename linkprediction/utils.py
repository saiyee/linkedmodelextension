...
def calc_raw_mrr(embedding, w, test_triplets, hits=[], eval_bz=100):
    ...
    with torch.no_grad():
        ...
        for hit in hits:
            ...
            results['hits@' + str(hit)] = avg_count.item()
...
def calc_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits=[], eval_bz=100, eval_p="filtered"):
    if eval_p == "filtered":
        results = calc_filtered_mrr(embedding, w, train_triplets, valid_triplets, test_triplets, hits)
    else:
        results = calc_raw_mrr(embedding, w, test_triplets, hits, eval_bz)
    return results
